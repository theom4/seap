...
interface OffersListProps {
  ...
  onClear: () => void
  onDelete?: (offer: OfferData, index: number) => void
}

export function OffersList({ webhookResponse, onClear, onDelete }: OffersListProps) {
        ...
        {offers.map((offer, index) => (
          ...
          >
            ...
            <div className="flex items-center space-x-2">
              ...
              {onDelete && (
                <button
                  onClick={() => onDelete(offer, index)}
                  className="px-3 py-2 bg-surface border border-red-900/50 text-red-400 rounded-lg hover:bg-red-900/30 hover:border-red-700 text-sm font-bold transition-all flex items-center space-x-1.5"
                  title="Șterge oferta"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
